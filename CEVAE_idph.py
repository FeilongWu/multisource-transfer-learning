...
def neur_network(insize, outsize, h, dh, act, zero_one = False, nonneg=None):
    ...
    if act.lower() == 'relu':
        ...
        actv = nn.Sigmoid()
    net = [nn.Linear(insize, dh), actv]
    for i in range(h):
        net.append(nn.Linear(dh, dh))
        net.append(actv)
    net.append(nn.Linear(dh, outsize))
    if zero_one:
        if outsize > 1:
...
class CEVAE(nn.Module):
    def __init__(self, nfcon, nfbin, d, device, h=3, prior_mean = torch.zeros(2), \
        ...
        # p(x1|z) continuous
        self.p_x1_z = Network(d, 2 * nfcon, h, dh, act, 'multiNormal').to(device)

        # p(x2|z) binary
        self.p_x2_z = Network(d, nfbin, h, dh, act, 'Bernoulli', zero_one=True).to(device)
        ...
        # q(t|x)
        self.q_t_x = Network(nfcon+nfbin, 1, h, dh, act, 'Bernoulli', zero_one=True).to(device)

        # shared representation of x
        self.rep_x = Network(nfcon+nfbin, dh, 0, dh, act, 'Normal', zero_one=True).to(device)
        ...
        self.rep_xy = Network(nfcon+nfbin+1, dh, 0, dh, act, 'Normal', zero_one=True).to(device)

        # q(z|x,y,t=0)
        self.q_z_xyt0 = Network(dh, 2 * d, h-1, dh, act, 'multiNormal').to(device)

        # q(z|x,y,t=1)

        self.q_z_xyt1 = Network(dh, 2 * d, h-1, dh, act, 'multiNormal').to(device)
    ...
    def getloss(self, xcon, xbin, y, t):
        x = torch.cat((xcon, xbin), 1)
        h_xy = self.rep_xy(torch.cat((x,y), 1))
        h_x = self.rep_x(x)
        z_mu, z_std = torch.chunk(torch.mul(self.q_z_xyt0(h_xy).T, 1-t).T + \
                                  torch.mul(self.q_z_xyt1(h_xy).T, t).T, 2, \
                                  dim=1)
        #z_std = self.multi_diag(z_std)
        sample_z = self.q_z_xyt0.sample(z_mu, torch.exp(0.5 * z_std)) # z_std = log var
        ...
        qy_mu = torch.mul(self.q_y_xt0(h_x).T, 1 - t).T + \
                                    torch.mul(self.q_y_xt1(h_x).T, t).T
        # logq (y|x,t=0)
        ...
        x1_mu, x1_std = torch.chunk(self.p_x1_z(sample_z), 2, dim=1)
        x1_std = self.multi_diag(torch.exp(x1_std))
        # logp(x1|z)
        log_x1_z = self.p_x1_z.log_p(xcon, x1_mu, x1_std)

        x2_mu = self.p_x2_z(sample_z)
        # logp(x2|z)
        log_x2_z = torch.mean(self.p_x2_z.log_p(xbin, x2_mu), 1)
        ...
        loss = - (log_x1_z +  log_x2_z + log_t_z + logp_y_zt + logp_z - logp_z_xyt\
                 + log_t_x + logq_y_xt)
        return loss
    
    def predict(self, xcon, xbin):
        x = torch.cat((xcon, xbin), 1)
        ...
        qz, _ = torch.chunk(torch.mul(self.q_z_xyt0(rep_xy).T, 1 - t).T + \
                            torch.mul(self.q_z_xyt1(rep_xy).T, t).T, 2, dim=1)
