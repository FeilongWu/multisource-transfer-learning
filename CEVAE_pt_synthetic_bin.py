...
class rep_xy(nn.Module):
    ...
    def forward(self, xy):
        return self.NN(xy).clamp(min=-1e06,max=1e07)
...
class rep_x(nn.Module):
    ...
    def forward(self, x):
        return self.NN(x).clamp(min=-1e06,max=1e07)
...
class CEVAE(nn.Module):
    def __init__(self, nf, dz, device, h=3, dl=100, act='elu'):
    ...
    def y_mean(self, z, t):
        dt = t.shape[0]
        return self.py0(z) * (1-t) + self.py1(z) * t
...
##        return y0.flatten(), y1.flatten()
        ...
        qt_loc = self.qt(x)
        qt_dist = dist.Bernoulli(qt_loc)
        t = qt_dist.sample(sample_shape = [sample]).view(sample*dt, 1)
        x = x.repeat(sample, 1)
        rep_x = self.rep_x(x)
        qy_loc = self.qy0(rep_x) * (1-t) + \
                 self.qy1(rep_x) * t
        ...
        qz_loc, qz_scale = torch.chunk(self.qz0(rep_xy) * (1-t) + \
                                ...
                                2, dim=1)
        qz_dist = dist.Normal(qz_loc, qz_scale)
        qz_samples = qz_dist.sample()
        y0 = torch.mean(self.py0(qz_samples).view(sample, dt, 1), dim=0).flatten()
        y1 = torch.mean(self.py1(qz_samples).view(sample, dt, 1), dim=0).flatten()
